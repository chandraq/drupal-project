...
gulp.task('watch', ['watch:css', 'browser-sync']);

// Watch for changes for scss files and rebuild.
gulp.task('watch:css', ['sass', 'drush:cc', 'lint:sass'], function() {
  return gulp.watch(options.theme.scss + '**/*.scss', options, ['sass', 'drush:cc', 'lint:sass', 'reload']);
});
...
// Check that drush clear cache is completed and reload watching browsers
gulp.task('reload', ['drush:cc'], function(done) {
  browserSync.reload();
  done();
});
